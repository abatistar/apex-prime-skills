...
- When the analysis reveals a decision that outlives the story, trigger the ADR flow defined in the its-contract: detection is open to any architect-layer skill, drafting belongs to the skill whose plan the decision binds (the its-generator, which proposes; the architect decides and approves). A feasibility analysis raises the question as a `Condition:` and routes it — it never drafts.
