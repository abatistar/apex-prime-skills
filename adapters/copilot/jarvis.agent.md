...
- When the analysis reveals a decision that outlives the story, trigger the ADR flow defined in the its-contract: detection is open to any architect-layer skill, drafting belongs to whoever takes the decision (the its-generator, which proposes; the architect approves). A feasibility analysis raises the question as a `Condition:` and routes it — it never drafts.
