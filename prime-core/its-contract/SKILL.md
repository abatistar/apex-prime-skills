...
This contract owns two: `templates/its-template.md` for the ITS, and `templates/adr-template.md` for the records an ITS references, whose rules live with the decision boundary below. Read in full the one you are about to write or validate.

The ITS's mandatory sections:
...
- **Reference glossary** — resolves every cited identifier whose definition lives in another document: `UC-NNN`, `RN-N`, `CA-N`, `ADR-NNN`, and the `QM-XX-N` or `CS-XX-N` that a recorded departure cites. For readers who do not have those documents at hand, plus the fixed process-term table. Delta items and regression points need no entry: this document defines them and they resolve in place. Metadata only (ID + name + one line) — never the referenced content. See Writing rules.
...
- When more than one use case is affected, qualify every identifier scoped to a use case with that use case: `RN-N` and `CA-N`, which the document owns, and `D1` and `RG-1`, which this ITS assigns per use case (`UC-042/RN-3`, `UC-042/RG-1`). An unqualified reference is ambiguous the moment a second document is in play.
...
- One table listing every cited identifier: ID, type, name, and a one-line gloss. For use cases, the Name field; for business rules and acceptance criteria, a one-line paraphrase of intent; for ADRs, the title, the status, and a one-line paraphrase of the **Decision** — what was decided, never why; for a quality criterion or a convention slot, its catalog name and its tier or class, never its statement.
...
- Regression → plan: every regression point (`RG-N`) is claimed by exactly one unit, which names the non-regression test it owes.
...
`templates/adr-template.md` is the canonical ADR structure, owned here for the same reason the ITS template is: one address, read by whoever drafts and whoever validates. Read it in full before drafting or approving a record. ADR location and naming come from `docs/prime-config.md`, defaulting to `docs/adr/ADR-NNN-short-title.md`, sequential and never renumbered. The its-generator detects candidate decisions and **proposes** the ADR; the architect approves before the ITS is delivered. An ITS embedding an architectural decision instead of referencing an ADR is not deliverable — same severity as a traceability failure. Detection is not exclusive to the its-generator: any architect-layer skill may surface that a decision is owed — `feasibility-analysis` does so as a `Condition:` when a demand challenges a recorded decision. **Drafting belongs where the decision becomes concrete.** An ADR is drafted by the skill whose own deliverable has to commit to the decision, which is why the its-generator is the only skill that drafts one: it is the only one that must write a plan the decision binds, unit by unit. It drafts as `Proposed`; the architect decides and approves. A skill that judges or maps commits to nothing, so it raises the question and routes it; it never files a Proposed ADR on the strength of an analysis whose own artifacts live outside the repository.

**The architect is never blocked by that rule.** It governs which *skill* may draft, not whether a decision can be recorded. A decision taken where no plan exists to commit to it — in architectural counsel, or at refinement before any story — is drafted by the architect directly, against this template and this lifecycle, and it needs no story to exist. What the rule forbids is a skill filing a record on the strength of an analysis it never had to build anything on. The person who took the decision is exactly who the record is owed by, and a decision that binds future work while waiting for a story to carry it is the invisible departure `QM-MT-1` names.
